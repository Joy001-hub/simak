...
class ValidateLicenseCommand extends Command
{
    ...
    public function handle(LicenseService $license)
    {
        ...
        // --- NEW: Check if last validation was within 7 days ---
        ...
        try {
            ...
            if ($isValid) {
                $license->saveLocalLicense(array_merge($local, ['license_key' => $key, 'status' => 'active', 'hardware_id' => $hardwareId, 'string' => $hardwareId, 'last_check_at' => now()->toIso8601String(), 'message' => 'Weekly validation successful',]));
            ...
            Cache::forever('app_offline_lock', true);
            $license->blockLocalLicense('Wajib connect internet untuk validasi mingguan.');
            $this->error('Offline Lock Activated');
            Notification::title('Security Alert')->message('Validation failed. App locked until online.')->show();
