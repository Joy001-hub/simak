    ...
    <div class="relative z-10 w-full max-w-md">
        <div class="bg-white/95 backdrop-blur rounded-2xl shadow-2xl border border-white/30 overflow-hidden">
            ...
            <div class="px-8 pt-8 pb-6 text-center border-b border-slate-200">
                ...
                <h1 class="text-2xl font-bold text-slate-900">Aktivasi Lisensi</h1>
                <p class="text-sm text-slate-600 mt-2">Masukkan email, password, dan license key Anda.</p>
            </div>
            ...
            {{-- Form --}}
            <form id="activate-form" action="{{ route('license.activate') }}" method="POST" class="px-8 py-8 space-y-5">
                @csrf
                <div>
                ...
                <button type="button" id="reset-btn"
                    class="w-full bg-slate-200 hover:bg-slate-300 text-slate-800 font-semibold py-3 rounded-lg transition duration-200 shadow-md hover:shadow-lg">
                    Reset Lisensi
                </button>
            </form>

            {{-- Hidden reset form --}}
            <form id="reset-form" action="{{ route('license.reset') }}" method="POST" class="hidden">
                @csrf
                <input type="hidden" name="email" id="reset-email">
                <input type="hidden" name="password" id="reset-password">
                <input type="hidden" name="license" id="reset-license">
            </form>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (resetBtn && resetForm) {
                resetBtn.addEventListener('click', () => {
                    ...
                    document.getElementById('reset-password').value = document.getElementById('password').value;
                    document.getElementById('reset-license').value = document.getElementById('license').value;
                    resetForm.submit();
                });
            }
        });
