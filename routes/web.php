...
Route::get('/login', function () {
    ...
    if (!$local || empty($local['license_key'])) {
        return redirect()->route('license.activate.form')->withErrors(['msg' => 'File lisensi tidak ditemukan. Silakan aktivasi lisensi baru.']); }return view('auth.login'); })->name('login');
Route::post('/login', [LicenseController::class, 'processAuthLogin'])->name('auth.login');
Route::get('/reset', function () {
    $licenseService = app(\App\Services\LicenseService::class);
    $local = $licenseService->loadLocalLicense();
    if (!$local || empty($local['license_key'])) {
        return redirect()->route('license.activate.form')->withErrors(['msg' => 'Tidak ada lisensi yang tersimpan. Silakan aktivasi terlebih dahulu.']); }return view('auth.resetpage'); })->name('reset');
...
Route::get('/native-img/logos/{filename}', function ($filename) {
    ...
    if (!file_exists($path)) {
        abort(404); }$file = file_get_contents($path);
